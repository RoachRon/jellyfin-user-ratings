(function () {
  try {
    ...
    const appRootPath = updootConfig.appRootPath;
    if (!appRootPath) {
      throw new Error('UPDOOT_CONFIG.appRootPath missing.');
    }
    const backendUrl = `${window.location.origin}${appRootPath}`;
    ...
    function createRecommendButton(playButton) {
      ...
      recommendButton.innerHTML =
        '<span class="material-icons thumb_up" aria-hidden="true"></span>';
      ...
      try {
        ...
        const targetContainer = document.querySelector(
          '.detailPagePrimaryContainer, .detailButton-container',
        );
        if (targetContainer) {
      ...
      const targetContainer = playButton.closest(
        '.mainDetailButtons, .detailButton-container, .detailPagePrimaryContainer',
      );
      if (targetContainer) {
          ...
          comments.forEach((comment) => {
            ...
            if (comment.userId === userId || adminUserIds.includes(userId)) {
              ...
              buttonGroup.style.cssText =
                'margin-left: 10px; display: flex; flex-direction: column; gap: 4px;';
    ...
    function getItemId() {
      ...
      if (!itemId) {
        const backdropImage = document.querySelector(
          '.backdropImage.displayingBackdropImage.backdropImageFadeIn',
        );
        if (backdropImage) {
    ...
    function createRecommendationsButton(castButton) {
      ...
      recommendationsButton.className =
        'headerButton btnRecommendations emby-button paper-icon-button-light';
      recommendationsButton.title = 'Recommendations';
      recommendationsButton.innerHTML =
        '<span class="material-icons star" aria-hidden="true"></span>'; // Fixed HTML syntax
      ...
      try {
        ...
        const topBar = document.querySelector(
          '.headerRight, .headerTabs, .mainDrawer-scrollContainer, .header',
        );
        if (topBar) {
    ...
    async function showRecommendationsOverlay() {
      ...
      try {
        ...
        overlay.style.display = 'flex';
        console.log(
          'Recommendations overlay displayed with',
          Object.keys(groupedByItem).length,
          'items',
        );
      } catch (error) {
        console.error('Error fetching recommendations:', error.message);
      ...
      try {
        ...
        deleteSelectedButton.addEventListener('click', async () => {
          ...
          const selectedIds = Array.from(
            commentSection.querySelectorAll('.commentCheckbox:checked'),
    ...
    function init() {
      ...
      function tryAddButtons() {
        ...
        const playButton = document.querySelector(
          '.mainDetailButtons .btnPlaystate, .detailButton-container button[data-id="play"]',
        );
        const castButton = document.querySelector(
          '.headerRight .headerCastButton, .headerTabs button[data-id="cast"], .mainDrawer-scrollContainer .castButton',
        );
    ...
    function setupNavigationListener() {
      ...
      const homeButton = document.querySelector(
        '.headerHomeButton, .skinHeader .emby-button[title="Home"]',
      );
      if (homeButton) {
